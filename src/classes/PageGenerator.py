...
class PageGenerator:
    ...
    def generate_pages_recursive(
            ...
            try:
                dir_entries: list[str] = FileHandler.list_dir_entries(dir_path_content)
                ...
                for dir_entry in dir_entries:
                    ...
                    if cur_path.endswith(".md") and FileHandler.is_file(cur_path):
                        ...
                        new_file_dest: str = os.path.join(
                            dest_path, cur_path.split("/")[-1].replace(".md", ".html")
                        )
                        
                        print(cur_path)
                        # Generate the page using the provided context
                        PageGenerator.generate_page(cur_path, template_path, new_file_dest)
                        continue

                    # If it's a directory, create the directory in the destination and recurse
                    new_dest_path = os.path.join(dest_path, cur_path.split("/")[-1])
